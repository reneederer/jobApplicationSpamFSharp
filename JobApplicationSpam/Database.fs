...
module Database =
    ...
    let getSentApplication (sentApplicationOffset : int) (userId : int) =
        ...
        let sentApplication =
            try
                query {
                    ...
                    join sentStatus in db.Sentstatus on (sentApplication.Id = sentStatus.Sentapplicationid)
                    where (sentApplication.Userid = userId && sentStatus.Sentstatusvalueid = 1)
                    sortBy sentApplication.Id
                    skip sentApplicationOffset
                    take 1
                    select
                        (Some
                            { jobName = sentDocument.Jobname
                              sentDate = sentStatus.Statuschangedon.ToShortDateString()
                              email = { subject = sentDocumentEmail.Subject; body = sentDocumentEmail.Body }
                              userEmail = sentUserValues.Email
                              userValues =
                                { gender = Gender.fromString sentUserValues.Gender
                                  degree = sentUserValues.Degree
                                  firstName = sentUserValues.Firstname
                                  lastName = sentUserValues.Lastname
                                  street = sentUserValues.Street
                                  postcode = sentUserValues.Postcode
                                  city = sentUserValues.City
                                  phone = sentUserValues.Phone
                                  mobilePhone = sentUserValues.Mobilephone
                                }
                              employer =
                              ...
                              filePages =
                                  db.Sentfilepage
                                    ...
                                    .OrderBy(fun (_, pageIndex) -> pageIndex)
                                  |> List.ofSeq
                            }
                        )
                } |> fun xs -> xs.Single()
        ...
        log.Debug (sprintf "(sentApplicationOffset = %i, userId = %i) = %A" sentApplicationOffset userId sentApplication)
        sentApplication
